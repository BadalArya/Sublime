...
void buildTree(int *arr, int *tree, int start, int end, int treeNode, int val){
	if(start == end){
		if(arr[start] < val){
			tree[treeNode] = 1;
		}else{
	...
	int mid = (start + end) / 2;
	buildTree(arr, tree, start, mid, 2 * treeNode, val);
	buildTree(arr, tree, mid+1, end, 2*treeNode + 1, val);
...
void Solve(){
    ...
    for(int i = 0; i < n; i++){
	    int *tree = new int[4*(n-i)];
	    buildTree(arr, tree, i, n - 1, 1, arr[i]);
