...
void Solve(){
	...
	if(count == 1){
		cout << max((n - lastOccurence) * 2, (lastOccurence + 1) * 2) << endl;
		return;
	}
	...
	int doubleVisit = 2 * (lastOccurence - firstOccurence + 1);
	int zero = 0;
	int res = max(firstOccurence - zero, n - 1 - lastOccurence);
	int ans1 = max(doubleVisit + res, n);
	int ans2 = max((1 + lastOccurence) * 2, (n - firstOccurence) * 2);
