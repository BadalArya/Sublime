...
void updateTree(int *tree, int *lazy, int start, int end, int treeNode, int left, int right){
	if(start > end){
		return;
	}
	...
	if(lazy[treeNode] != 0){
		if(lazy[treeNode] & 1){
			...
			tree[treeNode] = range - tree[treeNode];
		}
	...
	int mid = (start + end) / 2;
	updateTree(tree, lazy, start, mid, 2 * treeNode, left, right);
	updateTree(tree, lazy, mid + 1, end, 2 * treeNode + 1, left, right);
...
void Solve(){
	...
	for(int i = 0; i < k; i++){
		...
		if(operation == 0){
			updateTree(tree, lazy, 0, n - 1, 1, left, right);
		}else{
			cout << query(tree, lazy, 0 , n - 1, left, right, 1) << endl;
		}
	}
}
