...
// First thing that came to mind ki k - 1 tubes ko 2 size ka space dedo
// And last tube pr array par spirally traverse kar do
...
void Solve(){
	...
	for(int i = 0; i < n*m; i++){
		...
		index.push_back(make_pair(startRow, startColumn));
		// table[startRow - 1][startColumn - 1] = 1;
		if(leftToRight && startColumn <= m){
			startColumn++;
		}
		if(startColumn > m){
			startRow++;
	...
	for(int i = 0; i < 2*(k - 1); i += 2){
		cout << "2 ";
		cout << index[i].first << " " << index[i].second << " " << index[i+1].first << " " << index[i+1].second << endl;
		total -= 2;
	}
