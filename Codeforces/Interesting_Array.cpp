...
bool check(int *tree, int start, int end, int left, int right, int val, int treeNode){
	if(left > end || right < start){
		return true;
	}

	if(start >= left && end <= right){
		if((tree[treeNode]) == 2147483647){
			tree[treeNode] = val;
			return true;
		}else if((tree[treeNode] & val) == tree[treeNode]){
			return true;
		}else{
			return false;
		}
	...
	return (check(tree, start, mid, left, right, val, 2 * treeNode) && check(tree, mid+1, end, left, right, val, 2 * treeNode +1));
}
...
void Solve(){
	...
	for(int i = 0; i < m; i++){
		int left, right, val;
		cin >> left >> right >> val;
		if(!check(tree, 0, n - 1, left - 1, right - 1, val, 1)){
			cout << "-1" << endl;
			return;
		}else{
			ans[left - 1] = val;
